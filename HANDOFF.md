...
**Regenerated by Ahmed 2026-09-02.** The value in the transcript is dead. The
new value goes straight into Vercel as `CLICKBANK_API_KEY` and nowhere else.
...
after spend, trailing **7 days on transaction date** (so a refund issued today
on a six-week-old sale is caught). Manual: dashboard **Run**, or
`GET /api/reconcile?days=7&k=CRON_SECRET` (days ≤ 90 for a first backfill).

Two corrections, in order:

1. **Refunds and chargebacks → `status = 'reversed'`.** `type=RFND` and
   `type=CGBK`, `role=AFFILIATE`. A refund carries the original sale's receipt,
   which is what the postback stored in `network_txn_id`, so it is a direct
   match. Only rows not already reversed are touched, so re-runs count zero.
   `ad_performance` already excludes reversed rows; `tracker_health.revenue_reversed`
   starts meaning something.
2. **Sales the postback missed → inserted.** `type=SALE` receipts not present
   in `conversions` are written with `payout = Σ lineItemData.accountAmount`
   (the affiliate share, same number as `{affiliate_earnings}`), `sub_id` from
   ClickBank's stored `affSub1`, resolved to a `click_id` when it is one of our
   UUIDs, `ts` = ClickBank's `transactionTime`. A 23505 on insert means a
   postback landed in the same second and is ignored.

Response: `{ok, days, refunds, chargebacks, reversed, unmatched_reversals,
sales, missed_inserted}`. `unmatched_reversals` lists reversal receipts we hold
no row for — refunds of rebills (never recorded, by design) or sales that
predate the tracker. Listed, not acted on.

**Decisions baked in:**

- **Never fires CAPI.** ClickBank's integration owns Purchase. `check.mjs`
  fails if `sendConversion` appears in the file.
- **Customer identity is never persisted.** `orders2` returns name, email and
  address on every row. `raw` gets only a whitelist (`KEEP`: receipt, type,
  time, amount, vendor, affiliate, trackingId, currency; line items: itemNo,
  title, accountAmount, quantity). `check.mjs` fails if `email`, `lastName`,
  `postalCode` etc. ever enter that list.
- **Same fail-closed gate as spend.** Unset `CRON_SECRET` → 503, wrong key →
  403, unset `CLICKBANK_API_KEY` → 503. This is the one endpoint that could
  mark every sale reversed and CUT every winner; it is not allowed to be open.
- **ClickBank's JSON is XML-shaped.** One result arrives as an object, several
  as an array, none as an empty body. `arr()` normalises; `Page` is a request
  *header*; 206 = more pages, 200 = last.
