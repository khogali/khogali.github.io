const plans = [
    // Regular Plans
    {
        ...
        line9To12Price: 60,
        maxLines: 12,
        associatedPlans: ["Go5G Next 55+", "Go5G Next Military/First Responder"]
    },
    {
        type: "Go5G Plus",
        firstLinePrice: 95,
        ...
        line9To12Price: 50,
        maxLines: 12,
        associatedPlans: ["Go5G Plus 55+", "Go5G Plus Military/First Responder"]
    },
    {
        type: "Go5G",
        firstLinePrice: 80,
        ...
        line9To12Price: 40,
        maxLines: 12,
        associatedPlans: ["Go5G 55+", "Go5G Military/First Responder"]
    },
    {
        type: "Essentials",
        firstLinePrice: 65,
    ...
    // Hidden Plans for Result Display Only
    {
        type: "Go5G Next 55+",
        firstLinePrice: 85,
        secondLinePrice: 65,
...
function filterPlans() {
    ...
    const useInsiderCode = document.getElementById('use-insider-code').checked;

    const plan = plans.find(p => p.type === planType);
    
    let resultsHtml = generatePlanResult(plan, numLines, insuranceLines, applyAutoPay, addTaxes, useInsiderCode);

    // Check and display associated plans
    if (plan.associatedPlans) {
        plan.associatedPlans.forEach(associatedPlanType => {
            const associatedPlan = plans.find(p => p.type === associatedPlanType);
            resultsHtml += generatePlanResult(associatedPlan, numLines, insuranceLines, applyAutoPay, addTaxes, useInsiderCode);
        });
    }

    document.getElementById('plan-list').innerHTML = resultsHtml;
}

function generatePlanResult(plan, numLines, insuranceLines, applyAutoPay, addTaxes, useInsiderCode) {
    let totalPrice = calculateTotalPrice(plan, numLines, insuranceLines, applyAutoPay, addTaxes).totalPrice;

    // Apply Insider Code discount if selected
    if (useInsiderCode) {
    ...
    return `
        <div class="plan-card">
            <h2>${plan.type}</h2>
            <p><strong>Total Price: $${totalPrice.toFixed(2)}/mo</strong></p>
        </div>
    `;
}
